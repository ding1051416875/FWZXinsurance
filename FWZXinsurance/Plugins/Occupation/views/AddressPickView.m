...
- (instancetype)init:(Address *)address {
    self = [super init];
    if(self){
        self.frame = CGRectMake(0, kHeight, kWidth, 746/2);
...
- (void)initHeadScroll{
    ...
    self.headScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(30, 0, kWidth - 30,45)];
...
//    self.provinceLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightLight];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 45-1, kWidth, 1)];
...
- (void)didSelect:(NSInteger)index{
    ...
    [self.contentScrollView setContentOffset:CGPointMake(index*kWidth, 0) animated:YES];
    switch (index) {
        case 0:{
...
- (void)initContentScroll{
    ...
    self.contentScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 45, kWidth, 659/2)];
    ...
    self.contentScrollView.contentSize = CGSizeMake(kWidth * 3, 659/2);
    [self addSubview:self.contentScrollView];
    
    self.proviceTable = [[AddressPickTableView alloc] init];
    self.proviceTable.frame = CGRectMake(0, 0, kWidth, 659/2);
    ...
    self.proviceTable.addressPickBlock = ^(AddrObject *object){
        if(![object.code isEqualToString:weakSelf.address.provinceId]){
            weakSelf.address = [[Address alloc] init];
            weakSelf.address.provinceId = object.code;
            weakSelf.address.provinceName = object.name;
    ...
    self.cityTable.frame = CGRectMake(kWidth, 0, kWidth, 659/2);
    ...
    self.cityTable.addressPickBlock = ^(AddrObject *object){
        if(![object.code isEqualToString:weakSelf.address.cityId]){
            weakSelf.address.cityId = object.code;
            weakSelf.address.cityName = object.name;
            weakSelf.address.distId = nil;
            weakSelf.address.districtName =nil;
            [weakSelf updateHead];
    ...
    self.districtTable.frame = CGRectMake(kWidth * 2, 0, kWidth, 659/2);
    ...
    self.districtTable.addressPickBlock = ^(AddrObject *object){
        //确认选择
        weakSelf.address.distId = object.code;
        weakSelf.address.districtName = object.name;
        [weakSelf updateHead];
    ...
    [self.contentScrollView setContentOffset:CGPointMake(self.currentIndex*kWidth, 0) animated:YES];
}
